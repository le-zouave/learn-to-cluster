...
from ltc.utils import (read_meta, read_probs, l2norm, build_knns, knns2ordered_nbrs,
                   intdict2ndarray, rm_suffix, Timer)


class ClusterDataset(object):
    def __init__(self, cfg):
        dset_name = os.path.basename(cfg["dataset_path"])
        dset_path = cfg["dataset_path"]
        ...
        with Timer('read knn graph'):
            if knn_graph_path is not None:
                ...
                name = rm_suffix(os.path.basename(feat_path))   # e.g. halos_train
                knn_prefix = os.path.join(dset_path, "knns", name)
                knns = build_knns(knn_prefix, self.features, cfg.knn_method, cfg.knn)
    ...
    def __getitem__(self, index):
        ...
        if self.is_test:
            ...
            one_hop_idxs = torch.tensor(one_hop_labels, dtype=torch.float32)
